...
--*/
{
  ...
  //
  for (Index = 1; Index < PrimaryHeader->NumberOfPartitionEntries; Index++) {
    ...
    PartitionInfo.SignatureType   = HdDev.SignatureType;
...
--*/
{
  ...
  if (CompareMem (&PartHdr->Header.Signature, EFI_PTAB_HEADER_SIGNATURE, sizeof (UINT64)) != 0 ||
